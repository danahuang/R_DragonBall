...
missing_values <- sapply(test0, function(x) sum(is.na(x)))
null_count <- data.frame(Column =names(missing_values), 
                         Count = missing_values, 
                         Proportion = missing_values/nrow(test0)) %>%
  filter(Count > 0) %>%
  arrange(-Count)
test0 <- test0 %>% 
  select(-c(as.vector(null_count$Column[null_count$Proportion>0.8])))
...
## Output Data
##------------------------------------------------
write.csv(train0, file = "train_new2.csv", row.names = FALSE)
write.csv(test0, file = "test_new2.csv", row.names = FALSE)
